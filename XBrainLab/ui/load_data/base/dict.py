...
class LoadDict(LoadArray):
    ...
    def handle_dict(self, filepath, selected_data):
        if not self.dict_info.is_info_complete(selected_data):
            dict_info_module = DictInfoSetter(self, filepath, selected_data, self.dict_info, type_ctrl=self.type_ctrl.get())
        ...
        mne_data, generation_script = self.dict_info.generate_mne(filepath, selected_data, self.type_ctrl.get())
