...
class TimeEpoch(PreprocessBase):
    ...
    def __init__(self, parent, preprocessed_data_list):
        ...
        tk.Label(self, text="Choose Events: ").grid(
            row=0, column=0, sticky="w", padx=5
        )
        tk.Label(self, textvariable=self.field_var['select_events']).grid(
            row=0, column=1, sticky="w", padx=5, pady=5
        )
        tk.Button(self, text="...", command=self._choose_events, width=8).grid(
            row=0, column=2, padx=5, pady=5
        )
        tk.Label(self, text="Epoch limit start: ").grid(
            row=1, column=0, sticky="w", padx=5
        )
        tk.Entry(self, textvariable=self.field_var['epoch_tmin']).grid(
            row=1, column=1, sticky="ew", padx=5
        )
        tk.Label(self, text="Epoch limit end: ").grid(
            row=2, column=0, sticky="w", padx=5
        )
        tk.Entry(self, textvariable=self.field_var['epoch_tmax']).grid(
        ...
        tk.Button(self, text="Confirm", command=self._extract_epoch, width=8).grid(
            row=6, columnspan=3
        )
    ...
    def _choose_events(self):
        event_id = SelectEvents(
            self, self.preprocessor.get_preprocessed_data_list()
        ).get_result()
        if event_id:
