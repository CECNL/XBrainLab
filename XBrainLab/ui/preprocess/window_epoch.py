...
class WindowEpoch(PreprocessBase):
    ...
    def __init__(self, parent, preprocessed_data_list):
        ...
        tk.Label(self, text="Duration of each epoch (sec): ").grid(
            row=0, column=0, sticky="w", padx=5, pady=5
        )
        tk.Entry(self, textvariable=self.field_var['duration']).grid(
            row=0, column=1, sticky="ew", padx=5, pady=5
        )
        tk.Label(self, text="Overlap between epochs (sec): ").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        tk.Entry(self, textvariable=self.field_var['overlap']).grid(
