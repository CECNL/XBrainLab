...
class TimeEpoch(PreprocessBase):
    ...
    def get_preprocess_desc(self, baseline: list, selected_event_names: list, tmin: float, tmax: float):
        return f"Epoching {tmin} ~ {tmax} by event ({baseline} baseline)"

    def _data_preprocess(self, preprocessed_data: Raw, baseline: list, selected_event_names: list, tmin: float, tmax: float):
        ...
        for event_name in selected_event_id.keys():
            selection_mask = np.logical_or(selection_mask, raw_events[:,-1]==selected_event_id[event_name])
        selected_events = raw_events[selection_mask]
        if(len(selected_events) == 0):
            raise ValueError(f"No event markers found.")
